...
# August 2017
# Code for simulating data from the joint model with the current value association structure
...
invS <- function (t, u, i) {
  h <- function (s) {
    ...
    exp(log(phi) + (phi - 1) * log(s) + eta.t[i] + f1 * alpha0)
  }
...
# simulate censoring times from a uniform distribution, with the mean depending on group
