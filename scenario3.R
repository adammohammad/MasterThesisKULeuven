...
# August 2017
# Code for simulating data from the joint model with the cumulative effects association structure
...
invS <- function (t, u, i) {
  h <- function (s) {
    ...
    ###
    iBS <- ins(s, knots = Int_kn, Boundary.knots = Bound_kn)
    XXi <- cbind(group0*s, group1*s,Covariate*s, group0*iBS[, 1], group1*iBS[, 1],
                 group0*iBS[, 2], group1*iBS[, 2], group0*iBS[, 3], group1*iBS[, 3])
    ZZi <- cbind(s, iBS)
    f <- as.vector(XXi %*% betas + rowSums(ZZi * b[rep(i, nrow(ZZi)), ]))
    exp(log(phi) + (phi - 1) * log(s) + eta.t[i] + f * alpha3)
  }
...
#Ctimes[which(Ctimes>admCens)]=admCens
